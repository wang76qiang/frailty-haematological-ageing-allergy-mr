...
def build_corrected(insts: dict) -> pd.DataFrame:
    ...
    if not panu.empty:
        ...
        if len(miss) > 0.05 * len(all_rsids):
...
def run_trait(trait: str, inst: pd.DataFrame, panu: pd.DataFrame,
    ...
    if not sub.empty:
        ...
        # same selection set as the naive analysis (this trait's instruments)
        exp_corr = exp_corr[exp_corr["snp"].isin(set(inst["snp"]))].reset_index(drop=True)
